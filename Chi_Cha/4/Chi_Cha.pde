...
OscBus = new MidiBus(this, "Launchpad S", "MidiPipe Input 1");
n=0;
...
void noteOn(int channel, int pitch, int velocity) {
  ...
  if (midimapping[a][b]!=0){OscBus.sendNoteOn(1,midimapping[a][b],77); }
  println(midimapping[a][b]);
}
...
void noteOff(int channel, int pitch, int velocity) {
  ...
  if (midimapping[a][b]!=0){OscBus.sendNoteOff(1,midimapping[a][b],77); }
}


void LoadBoard(){
String[] strLines = loadStrings("board.txt");
...
String[] strLines = loadStrings("midimapping.txt");
for (int i = 0; i < strLines.length; i=i+1) {
  int[] xline = int(split(strLines[i], ',')); 
for (int c =0; c <=nx;c=c+1) {
